<div align="center">

<img src="https://profileforge-one.vercel.app/api/widgets/banner?style=gradient&text=your-username&theme=midnight" alt="your-username" width="100%" />

# Hi there, I'm your-username 👋

### Available for select projects

</div>

## Services

- 🚀 MVPs in weeks, not quarters
- 🧹 Rescues of codebases other people gave up on
- 📈 Performance audits with receipts

<img src="https://profileforge-one.vercel.app/api/widgets/divider?style=wave&theme=midnight" alt="" width="100%" />

## 🛠️ Tech Stack

<div align="center">
  <img src="https://img.shields.io/badge/TypeScript-3178C6?style=for-the-badge&logo=typescript&logoColor=white" alt="TypeScript" />
  <img src="https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=nextdotjs&logoColor=white" alt="Next.js" />
  <img src="https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB" alt="React" />
  <img src="https://img.shields.io/badge/Node.js-339933?style=for-the-badge&logo=nodedotjs&logoColor=white" alt="Node.js" />
  <img src="https://img.shields.io/badge/MongoDB-47A248?style=for-the-badge&logo=mongodb&logoColor=white" alt="MongoDB" />
  <img src="https://img.shields.io/badge/AWS-232F3E?style=for-the-badge&logo=amazonwebservices&logoColor=white" alt="AWS" />
</div>

<div align="center">
  <img src="https://profileforge-one.vercel.app/api/widgets/stats?theme=midnight&user=your-username" alt="GitHub Stats" />
</div>

<div align="center">
  <img src="https://profileforge-one.vercel.app/api/widgets/activity?theme=midnight&user=your-username" alt="Contribution activity graph" width="100%" />
</div>

<img src="https://profileforge-one.vercel.app/api/widgets/divider?style=wave&theme=midnight" alt="" width="100%" />

## 💼 Selected Work

| Project | Description | Stack |
| --- | --- | --- |
| **[client-saas](https://github.com/your-username/client-saas)** | 0→launch in 6 weeks | `nextjs` `mongodb` |
| **[perf-rescue](https://github.com/your-username/perf-rescue)** | LCP 4.2s → 1.1s case study | `react` |
| **[api-rebuild](https://github.com/your-username/api-rebuild)** | Legacy PHP → typed Node | `nodejs` `typescript` |

## 📬 Work with me

**Booking for next month** — email me or reach out on LinkedIn below.

<div align="center">
  <img src="https://profileforge-one.vercel.app/api/widgets/quote?theme=midnight" alt="Dev quote of the day" />
</div>

## 🌐 Connect with me

<div align="center">
  <a href="https://github.com/your-username"><img src="https://img.shields.io/badge/GitHub-181717?style=for-the-badge&logo=github&logoColor=white" alt="GitHub" /></a>
  <a href="https://linkedin.com/in/your-username"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
</div>

---

<div align="center">
  <sub>⚡ Crafted with <a href="https://github.com/Dev-Shivam-05/GitHub-Contribution-Art-Generator"><b>ProfileForge</b></a> — build a profile like this in minutes</sub>
</div>
